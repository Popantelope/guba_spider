"""
Created on Thurs Jan 22 12:26:43 2023
...
def get_content_caifuhao(bro, url):
    '''
...
def get_comments_caifuhao(bro, url, postid, writer, data_info):
    '''
        ...
        try:
            ...
            for div in div_list[1:-2:]:
                ...
                if referer[8:12:] == 'guba':
                    # 获取帖子内容
                    temp = get_content_guba(bro, referer)
                    ...
                    temp = get_content_caifuhao(bro, referer)
                    data_info['time'] = temp[0]
                    data_info['content'] = temp[1]
                    try:
                        if int(div.xpath('./span[2]/text()')[0]) == 0:
                            ...
                            get_comments_caifuhao(bro, referer, postid, writer, data_info)
                    except:
                        get_comments_caifuhao(bro, referer, postid, writer, data_info)
                # 有些帖子是视频，遇到直接跳过
                else:
                    continue
                print(code, div.xpath('./span[3]/a/@title')[0], '写入成功！！！')
        except:
            continue
    print(filepath, '写入完成！！！')
    print('===============================================================================')
...
if __name__ == '__main__':
    ...
    # 获取股票列表
    Codes = pd.read_excel('../Code_1_1 - 副本.xlsx', dtype='object')
    codes = list(Codes['Code'])
    # codes = ['000517', '000528', '000539', '000530']
    # 开启多进程（4进程）
    pool = Pool(4)  # 进程开多了跑久了selenium会报Timeout错误（我开8个和6个就报错了）
